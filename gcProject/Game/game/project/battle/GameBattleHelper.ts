...
class GameBattleHelper {
     ...
     * @return [number] 等级
     */
    static getLevelByActor(actor: Module_角色): number {
     ...
     */
    static get allBattlers(): ProjectSceneObject_1[] {
        var arr = [];
        for (var i = 0; i < Game.currentScene.sceneObjects.length; i++) {
            var so = Game.currentScene.sceneObjects[i];
     ...
     * @return [boolean]  
     */
    static isBattler(so: ProjectSceneObject_1): boolean {
     ...
     * @return [boolean] 
     */
    static isPlayerCamp(so: ProjectSceneObject_1): boolean {
     ...
     * @return [boolean] 
     */
    static isEnemyCamp(so: ProjectSceneObject_1): boolean {
     ...
     * @return [boolean] 
     */
    static isInPlayerParty(so: ProjectSceneObject_1): boolean {
     ...
     * @return [boolean] 
     */
    static isFriendlyRelationship(so1: ProjectSceneObject_1, so2: ProjectSceneObject_1): boolean {
     ...
     * @return [boolean] 
     */
    static isHostileRelationship(so1: ProjectSceneObject_1, so2: ProjectSceneObject_1): boolean {
        return this.isBattler(so1) && this.isBattler(so2) && so1.battlerSetting.battleCamp != so2.battlerSetting.battleCamp;
    }
    /**
     * 是否玩家可控制的战斗角色
     */
    static isPlayerControlEnabledBattler(so: ProjectSceneObject_1): boolean {
     ...
     * @return [boolean] 
     */
    static isHostileSkill(skill: Module_技能): boolean {
     ...
     * @return [boolean] 
     */
    static isFriendlySkill(skill: Module_技能): boolean {
     ...
     * @return [boolean] 
     */
    static canUseOneItem(battler: ProjectSceneObject_1, item: Module_道具): boolean {
        // 非战斗者不允许
     ...
     * @return [boolean] 
     */
    static getBattlerStatus(battler: ProjectSceneObject_1, statusID: number): Module_状态 {
     ...
     * @return [boolean] 
     */
    static isIncludeStatus(battler: ProjectSceneObject_1, statusID: number): boolean {
     ...
     * @return [boolean] 
     */
    static canSuperpositionLayer(battler: ProjectSceneObject_1, statusID: number): boolean {
        var status: Module_状态 = ArrayUtils.matchAttributes(battler.battlerSetting.battleActor.status, { id: statusID }, true)[0];
     ...
     * @return [boolean] 
     */
    static canMove(battler: ProjectSceneObject_1): boolean {
        // 非战斗者不允许
        if (!GameBattleHelper.isBattler(battler)) return false;
     ...
     * @return [boolean] 
     */
    static canAttack(battler: ProjectSceneObject_1): boolean {
        // 非战斗者不允许
        if (!GameBattleHelper.isBattler(battler)) return false;
     ...
     * @return [boolean] 
     */
    static canUseSkill(battler: ProjectSceneObject_1): boolean {
        // 非战斗者不允许
        if (!GameBattleHelper.isBattler(battler)) return false;
        // 已待机或死亡的话则不允许
        if (battler.battlerSetting.operationComplete || battler.battlerSetting.isDead) return false;
     ...
     * @return [boolean] 
     */
    static canUseOneSkill(battler: ProjectSceneObject_1, skill: Module_技能, checkUseSkillCommconCondition: boolean = true): boolean {
        // 非战斗者不允许
     ...
     * 获取光标对象
     */
    static get cursor(): ProjectSceneObject_1 {
     ...
     * @return [ProjectSceneObject_1] 
     */
    static getSceneObjectByGrid(posGridP: Point, ignoreDeadUnit: boolean = false): ProjectSceneObject_1 {
        // 越界
     ...
     * 获取场景光标上的场景对象
     */
    static get overCursorSceneObject(): ProjectSceneObject_1 {
        // 获取光标位置（格子）
     ...
     * @return [ProjectSceneObject_1] 
     */
    static getNoDeadBattlerByGrid(posGridP: Point): ProjectSceneObject_1 {
     ...
     * 获取场景光标上的战斗者对象
     */
    static get overCursorNoDeadBattler(): ProjectSceneObject_1 {
        var p = GameBattleHelper.cursor;
        var posGridP = new Point(p.posGrid.x, p.posGrid.y);
     ...
     * 获取场景光标上的玩家阵营的战斗者对象
     */
    static get overCursorPlayerCampNoDeadBattler(): ProjectSceneObject_1 {
     ...
     * 获取场景光标上的敌对阵营的战斗者对象
     */
    static get overCursorEnemyCampNoDeadBattler(): ProjectSceneObject_1 {
        var overCursorNoDeadBattler = this.overCursorNoDeadBattler;
     ...
     */
    static get overCursorPlayerCtrlEnabledBattler(): ProjectSceneObject_1 {
     ...
     * @param gridPos 指定的格子坐标
     */
    static getAttackTargetOnGrid(battler: ProjectSceneObject_1, gridPos: Point): ProjectSceneObject_1 {
        // 不在作用范围则忽略
     ...
     */
    static getSkillTargetOnGrid(fromBattler: ProjectSceneObject_1, skill: Module_技能, gridPos: Point): { allow: boolean, targets: ProjectSceneObject_1[] } {
        // 不在作用范围则忽略（全体技能除外）
        ...
        // 目标类别：队友多体、敌人多体
            ...
            var targets: ProjectSceneObject_1[] = [];
     ...
     * @return [ProjectSceneObject_1] 
     */
    static get nextPlayerControlBattler(): ProjectSceneObject_1 {
     ...
     * @return [ProjectSceneObject_1] 
     */
    static get nextPlayerCampComputerControlBattler(): ProjectSceneObject_1 {
        for (var i = 0; i < GameBattle.playerBattlers.length; i++) {
            var playerBattler = GameBattle.playerBattlers[i];
            if (playerBattler.battlerSetting.isDead) continue;
            if (!GameBattleHelper.isPlayerControlEnabledBattler(playerBattler) && !playerBattler.battlerSetting.operationComplete) {
     ...
     * 获取下一个敌方控制的角色
     */
    static get nextEnemyControlBattler(): ProjectSceneObject_1 {
     ...
     * @return [Point] 
     */
    static getSkillEffectRangeGrid(posGrid: Point, skill: Module_技能): Point[] {
     ...
     * @param posGrid 起始点
     * @param range 范围
     * @param obstacleMode [可选] 默认值=0 计算障碍的模式 0-计算所有障碍 1-仅计算地图固定障碍 2-不计算障碍
    ...
    static getEffectRange(firstGrid: Point, range: number, obstacleMode: number = 0, rangeMode: number = 0, minRange: number = 0,
        customRangeData: { size: number, gridData: number[][] } = null, isThroughObstacle: boolean = false): Point[] {
        var calcDistance = rangeMode != 2;
        var battlerThroughGridMap = this.getDestinationThroughGridMap(firstGrid, range, obstacleMode, calcDistance, customRangeData);
     ...
     * @return [Point] 相对位置集合 
     */
    static getSkillReleaseRangeGrid(skill: Module_技能): Point[] {
        // 获得释放范围格子数据
    ...
    static getDestinationThroughGridMap(posGrid: Point, range: number, obstacleMode: number = 0, caleDistance: boolean = true,
        customRangeData: { size: number, gridData: number[][] } = null): { grids: Point[], throughGridMap: boolean[][] } {
        var grids: Point[] = [];
        // 首个格子
            ...
            for (var x = startX; x <= endX; x++) {
                ...
                for (var y = startY; y <= endY; y++) {
                    ...
                    if (firstGrid.x != x || firstGrid.y != y) {
                        if (obstacleMode == 0) {
                            var isObstacleGrid = Game.currentScene.sceneUtils.isObstacleGrid(ProjectUtils.pointHelper);
                        }
     ...
     * @return [Animation] 
     */
    static createEffectGrid(gridX: number, gridY: number, mode: number): Animation {
        if (mode == 0) {
     ...
     * @param battler2 战斗者2
     */
    static getBattlerDistance(battler1: ProjectSceneObject_1, battler2: ProjectSceneObject_1) {
    ...
    */
    static calculationHitResult(fromBattler: ProjectSceneObject_1, targetBattler: ProjectSceneObject_1, isHitSuccess: boolean, actionType: number, skill: Module_技能 = null, item: Module_道具 = null, status: Module_状态 = null): {
        damageType: number,
        damage: number,
        isCrit: boolean
    } {
