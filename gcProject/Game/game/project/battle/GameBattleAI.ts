...
class GameBattleAI {
     ...
     * 当前战斗者
     */
    private static currentBattler: ProjectSceneObject_1;
    /**
     * 参考目标敌人
     */
    private static currentEnemy: ProjectSceneObject_1;
    /**
     * 锁定一次敌人
     */
    private static lockOnceEnemy: ProjectSceneObject_1;
    /**
     * 正在行动中
     */
     ...
     */
    static action(battler: ProjectSceneObject_1, onActionComplete: Callback) {
        // -- 记录当前行动的战斗者
     ...
     * 获取当前操作的战斗者的角色数据
     */
    private static get currentBattlerActor(): Module_角色 {
    ...
    private static tryUseSkill() {
        ...
        for (var i = 0; i < actor.skills.length; i++) {
            ...
            new SyncTask(this.doActionSyncTask, (skill: Module_技能) => {
                // 已有技能使用的情况直接结束该任务
                if (isUseSkill) {
                    SyncTask.taskOver(this.doActionSyncTask);
                    return;
                ...
                var skillTarget: ProjectSceneObject_1 = this.getSkillTarget(skill, isHostileSkill);
                // 没有目标的情况
                if (!skillTarget) {
                    SyncTask.taskOver(this.doActionSyncTask);
                    return;
        ...
        new SyncTask(this.doActionSyncTask, () => {
            ...
            if (!this.isInAction && GameBattleHelper.canAttack(this.currentBattler) && this.currentEnemy) {
                ...
                if (actor.atkMode == 1 && actor.atkSkill) {
                    ...
                    var skillTarget: ProjectSceneObject_1 = this.getSkillTarget(actor.atkSkill, isHostileSkill);
                ...
                // 普通攻击
                    ...
                    var atkTarget: ProjectSceneObject_1 = this.getAtkTarget();
                    if (atkTarget) {
     ...
     * @return [boolean] 成功使用
     */
    private static doUseSkill(skill: Module_技能, skillTarget: ProjectSceneObject_1): boolean {
        // 不存在技能或目标的话，使用失败
     ...
     * @param item 
     */
    private static doUseItem(item: Module_道具) {
        this.isInAction = true;
        // 打开技能指示器
        ...
        setTimeout(() => {
            ...
            // -- 使用道具
            GameBattleAction.useItem(this.currentBattler, item);
        }, WorldData.actionReflectionTime);
    }
    /**
     * 执行攻击指令
     ...
     */
    private static getOneEnemy(ignoreEnemys: ProjectSceneObject_1[] = []): ProjectSceneObject_1 {
        var actor = this.currentBattler.battlerSetting.battleActor;
        ...
        if (isGetNearEmeny) {
            ...
            var minDistanceEnemy: ProjectSceneObject_1 = null;
     ...
     */
    private static getSkillTarget(skill: Module_技能, isHostileSkill: boolean): ProjectSceneObject_1 {
        var realTarget: ProjectSceneObject_1;
        var currentBattlerActor = this.currentBattlerActor;
        // 获取目标组
        var targetArr: ProjectSceneObject_1[] = [];
        // -- 如果是针对敌方的技能
        if (isHostileSkill) {
     ...
     * 获取一个攻击目标
     */
    private static getAtkTarget(): ProjectSceneObject_1 {
        // -- 已在最近的距离则直接返回
     ...
     */
    private static getSkillCastPlace(skill: Module_技能, target: ProjectSceneObject_1, skillFirstGrid: Point = null): { castPoint: Point, targets: ProjectSceneObject_1[] } {
        // -- 全体技能，直接获取
        if (skill.targetType == 3 || skill.targetType == 4) {
        ...
        // -- 当前战斗者所在位置能够释放的点集合
        var currentPositionCastArr: { castPoint: Point, targets: ProjectSceneObject_1[] }[] = [];
        // -- 必须指定空地的话则随机选择一个空地格子
        ...
        if (!skillFirstGrid && GameBattleHelper.canMove(this.currentBattler)) {
            var moveMapGrids = GameBattleHelper.getEffectRange(this.currentBattler.posGrid, this.currentBattler.battlerSetting.battleActor.MoveGrid);
            var preResArr: { castPoint: Point, targets: ProjectSceneObject_1[], moveMapGrid: Point }[] = [];
            for (var s = 0; s < moveMapGrids.length; s++) {
                var moveMapGrid = moveMapGrids[s];
        ...
        // 内部方法：获取最大性价比的施放地点
        function getBestCastPoint(castPointArr: { castPoint: Point, targets: ProjectSceneObject_1[] }[]) {
     ...
     * 获取技能的最大作用距离范围
     */
    static getSkillMaxEffectDistance(skill: Module_技能): number {
