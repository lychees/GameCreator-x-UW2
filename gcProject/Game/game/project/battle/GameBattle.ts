...
class GameBattle {
     ...
     * 我方战斗成员
     */
    static playerBattlers: ProjectSceneObject_1[] = [];
    /**
     * 敌方战斗成员
     */
    static enemyBattlers: ProjectSceneObject_1[] = [];
    //------------------------------------------------------------------------------------------------------
    // 结果
    //------------------------------------------------------------------------------------------------------
    /**
    ...
    static start(): void {
        ...
        // 战斗正式开始时事件
        CommandPage.startTriggerFragmentEvent(WorldData.afterStartBattleEvent, Game.player.sceneObject, Game.player.sceneObject, Callback.New(() => {
            // 恢复列表快捷键
    ...
    private static firstCampStartActionStep(): void {
        // 进入先行方行动的事件处理
        CommandPage.startTriggerFragmentEvent(WorldData.battleStage2_firstCampStartAction, Game.player.sceneObject, Game.player.sceneObject, Callback.New(() => {
            // 决定玩家行动或敌方行动
    ...
    private static secondCampStartActionStep(): void {
        // 进入后行方行动的事件处理
        CommandPage.startTriggerFragmentEvent(WorldData.battleStage3_secondCampStartAction, Game.player.sceneObject, Game.player.sceneObject, Callback.New(() => {
            // 决定玩家行动或敌方行动
    ...
    private static settlementStep(): void {
        ...
        // 进入结算阶段的事件处理
        CommandPage.startTriggerFragmentEvent(WorldData.battleStage4_settlement, Game.player.sceneObject, Game.player.sceneObject, Callback.New(() => {
            // 战场判定处理
            this.battlerfieldDetermineHandle(() => {
     ...
     * @param onFin 
     */
    static checkBattlerIsDead(battler: ProjectSceneObject_1, onFin: Function): void {
        // 当生命值归零的时候
        if (!battler.battlerSetting.isDead && battler.battlerSetting.battleActor.hp == 0) {
            // 执行死亡者事件
            CommandPage.startTriggerFragmentEvent(WorldData.whenBattleDeadEvent, battler, battler, Callback.New(() => {
                // -- 刷新参战者所在的阵营以及检查新战斗者加入
                GameBattlerHandler.refreshCampAndNewBattles();
                // -- 仍然是战斗者且死亡的话则进行死亡处理
    ...
    private static getBattleCompleteState(): number {
        ...
        for (var i = 0; i < GameBattle.setting.addFailConditions.length; i++) {
            ...
            var thisActorBattlers: ProjectSceneObject_1[] = ArrayUtils.matchAttributesD3(this.playerBattlers, "battlerSetting", "battleActor", { id: actorID }, false);
            // -- 获取其中的死亡角色
            var thisActorDeadBattlers: ProjectSceneObject_1[] = ArrayUtils.matchAttributesD2(thisActorBattlers, "battlerSetting", { isDead: true }, false);
            // -- 查询这些死亡角色是否是玩家拥有的角色
