...
class GameBattleAction {
     ...
     * 当前行为的战斗者
     */
    static fromBattler: ProjectSceneObject_1;
    /**
     * 当前行为的战斗者使用的技能
     */
    static fromBattlerSkill: Module_技能;
    /**
     * 当前行为的战斗者施放的地点
     */
     ...
     * 当前行为的战斗者使用的道具
     */
    static fromBattlerItem: Module_道具;
    /**
     ...
     * 战斗者作用范围格子集
     */
    private static battlerEffectGridAniArr: Animation[] = [];
    /**
     * 战斗者释放范围格子集
     */
    private static battlerEffectReleaseGridAniArr: Animation[] = [];
    /**
     * 开启释放范围
     */
     ...
     */
    static cameraMoveToBattler(battler: ProjectSceneObject_1): { useTweenTime: boolean; cameraLockChange: boolean; } {
    ...
    static cursorJumpToGridPoint(grid: Point): void {
        ...
        if (useTweenTime) GameFunction.cameraMove(1, 0, 0, cursor.index, true, useTweenTime ? WorldData.cameraTweenFrame : 1);
    }
    //------------------------------------------------------------------------------------------------------
    // 界面
    //------------------------------------------------------------------------------------------------------
     ...
     * @param battler 当前战斗者 
     */
    static showCurrentBattlerWindow(battler: ProjectSceneObject_1): void {
     ...
     * @param battler 目标战斗者
     */
    static showTargetBattlerWindow(battler: ProjectSceneObject_1): void {
     ...
     */
    static startMove(battler: ProjectSceneObject_1, toGridX: number, toGridY: number, onComplete: Callback = null) {
        // 记录当前行为的战斗者
     ...
     */
    static attack(fromBattler: ProjectSceneObject_1, targetBattler: ProjectSceneObject_1): void {
     ...
     */
    static useSkill(fromBattler: ProjectSceneObject_1, skill: Module_技能, gridPos: Point, targets: ProjectSceneObject_1[] = null, firstUse: boolean = true) {
        ...
        // 显示当前攻击者和目标的界面
        this.showCurrentBattlerWindow(fromBattler);
        var onlyOneTarget: ProjectSceneObject_1 = null;
        if (targets.length == 1) {
        ...
        if (firstUse) {
            ...
            // 执行片段事件-战斗过程：使用技能
            if (WorldData.battleProcess_useSkill) CommandPage.startTriggerFragmentEvent(WorldData.battleProcess_useSkill, fromBattler, onlyOneTarget ? onlyOneTarget : fromBattler, Callback.New(doCallSkillEvent, this));
            else doCallSkillEvent.apply(this);
        }
        else {
     ...
     * @param item 道具
     */
    static useItem(fromBattler: ProjectSceneObject_1, item: Module_道具): void {
        ...
        var doUseItem = () => {
            ...
            GameBattle.battlerfieldDetermineHandle(() => {
                ...
                this.hitTarget(fromBattler, fromBattler, 2, null, item);
            });
        }
        // 执行片段事件-战斗过程：使用道具
        if (WorldData.battleProcess_useItem) CommandPage.startTriggerFragmentEvent(WorldData.battleProcess_useItem, fromBattler, fromBattler, Callback.New(doUseItem, this));
        else doUseItem.apply(this);
    }
    //------------------------------------------------------------------------------------------------------
    // 结算阶段
    //------------------------------------------------------------------------------------------------------
    ...
    static calcBattlersStatus(onFin: Callback): void {
        ...
        for (var i = 0; i < allBattlers.length; i++) {
            ...
            for (var s = 0; s < battlerActor.status.length; s++) {
                var status = battlerActor.status[s];
                if (!status.overtime) continue;
                new SyncTask(taskName, (battler: ProjectSceneObject_1, status: Module_状态) => {
     ...
     */
    private static releaseSkill(fromBattler: ProjectSceneObject_1, skill: Module_技能, gridPos: Point, targets: ProjectSceneObject_1[], firstUse: boolean) {
        // 等待一次击中算作完成行为
        this.currentHitTarget = 0;
     ...
     */
    private static releaseBullet(fromBattler: ProjectSceneObject_1, skill: Module_技能, gridPos: Point, targetBattler: ProjectSceneObject_1 = null, targets: ProjectSceneObject_1[]) {
        // 是否击中地面
        var isHitGround = skill.targetType >= 5 && skill.targetType <= 6;
        // 创建子弹动画
     ...
     */
    private static hitGround(fromBattler: ProjectSceneObject_1, skill: Module_技能, gridPos: Point, targets: ProjectSceneObject_1[]): void {
        // 击中后处理
        ...
        var doHitOpenSpace = () => {
            ...
            GameBattle.battlerfieldDetermineHandle(() => {
                ...
                if (hitAniID) {
                    ...
                    hitAni.play();
                    hitAni.once(Animation.PLAY_COMPLETED, this, () => {
                        if (!alreadyActionComplete) afterHitOpenSpace.apply(this);
                        hitAni.dispose();
                    });
                    // 监听动画抛出的信号，以便提前进入显示伤害阶段
                    hitAni.once(Animation.SIGNAL, this, (signalID: number) => {
                        if (signalID == 1) {
     ...
     */
    private static hitTarget(fromBattler: ProjectSceneObject_1, targetBattler: ProjectSceneObject_1, actionType: number, skill: Module_技能 = null, item: Module_道具 = null, status: Module_状态 = null): void {
        // 获取战斗者的角色数据
     ...
     */
    private static hitResult(fromBattler: ProjectSceneObject_1, targetBattler: ProjectSceneObject_1, isHitSuccess: boolean, actionType: number, skill: Module_技能 = null, item: Module_道具 = null, status: Module_状态 = null): void {
        // 等待播放效果播放完毕后算作「行动完成」
     ...
     * @param onFin 当完成时回调
     */
    private static hitByStatus(battler: ProjectSceneObject_1, status: Module_状态, onFin: Callback): void {
     ...
     */
    static showDamage(targetBattler: ProjectSceneObject_1, damageType: number, damage: number = 0, isCrit: boolean = false, onFin: Callback = null): void {
        ...
        if (uiID != 0) {
            ...
            if (targetUI) {
                ...
                damageAni.once(Animation.PLAY_COMPLETED, this, () => {
                    damageAni.dispose();
                    damageUI.dispose();
                    if (lastRelease) {
                        onFin && onFin.run();
     ...
     * 开启技能指示器
     */
    static openSkillIndicator(battler: ProjectSceneObject_1, skill: Module_技能): void {
        // 被动技能
     ...
     * 打开道具指示器
     */
    static openItemIndicator(battler: ProjectSceneObject_1): void {
        var itemRange = 0;
     ...
     * 打开战斗者的技能作用范围
     */
    private static openBattlerSkillEffectRangeGird(battler: ProjectSceneObject_1, skill: Module_技能) {
        // 关闭作用范围
        this.closeBattlerEffectRangeGrid();
        // 获得作用范围格子数据
     ...
     */
    private static openBattlerEffectRangeGrid(battler: ProjectSceneObject_1, range: number, obstacleMode: number = 0, rangeMode: number = 0, minRange: number = 0,
        customRangeData: { size: number, gridData: number[][] } = null): Point[] {
        // 关闭作用范围
        this.closeBattlerEffectRangeGrid();
        // 获得作用范围格子数据
     ...
     * 打开释放范围
     */
    private static openBattlerReleaseRangeGrid(skill: Module_技能): Point[] {
        // 关闭作用范围
        this.closeBattlerReleaseRangeGrid();
