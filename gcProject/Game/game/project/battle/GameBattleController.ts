...
class GameBattleController {
     ...
     * 记录移动过的战斗者
     */
    private static recordMovedBattler: ProjectSceneObject_1;
    /**
     * 记录移动者原面向
     */
    ...
    /**
      * 当前玩家操作的战斗角色
      */
    static currentOperationBattler: ProjectSceneObject_1;
    /**
     * 当前控制角色的战斗方式 0-普通攻击 1-使用技能 2-使用道具
     */
    private static currentBattleType: number = 0;
    /**
     * 当前控制角色的技能
     */
    private static currentBattleSkill: Module_技能;
    /**
     * 当前控制角色的道具
     */
    private static currentBattleItem: Module_道具;
    //------------------------------------------------------------------------------------------------------
    // 实现用的变量
    //------------------------------------------------------------------------------------------------------
    ...
    static battleCommand_controlComplete(skipAIOperaction: boolean) {
        var nextPlayerControlBattler: ProjectSceneObject_1;
    ...
    static battleCommand_openAtkIndicator(): void {
        ...
        var atkSkill: Module_技能 = battler.battlerSetting.battleActor.atkSkill;
     ...
     * @param skill 技能
     */
    private static battleCommand_onSkillSelect(skill: Module_技能) {
        var battler = this.currentOperationBattler;
        // 关闭战斗菜单
     ...
     * @param item 技能
     */
    private static battleCommand_onItemSelect(item: Module_道具): void {
        var battler = this.currentOperationBattler;
        // 关闭战斗菜单
        this.closeBattlerMenu();
        // 记录当前选中的道具
     ...
     * @param battler 
     */
    static openBattlerMenu(battler: ProjectSceneObject_1) {
        // 记录当前的操作对象
    ...
    private static onEnterFrame(pointGrid: Point = null, mouseMode: boolean = false): void {
        ...
        // 否则如果存在光标指向的目标时仅显示光标指向的目标
            ...
            if (mouseMode) {
                if (battler) GameBattleAction.showTargetBattlerWindow(battler);
                return;
            }
            if (battler) {
    ...
    private static selectBattleTargetOrArea() {
        ...
        // 道具
            ...
            if (target != this.currentOperationBattler) return;
            // -- 等待行为结束后重新操作角色
            this.whenBattleActionCompleteReOpenBattleMenu();
            // -- 使用道具
