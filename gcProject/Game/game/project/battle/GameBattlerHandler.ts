...
class GameBattlerHandler {
     ...
     * @param forceBin
     */
    static onBattlerAppear(battler: ProjectSceneObject_1): void {
        // 忽略非战斗者
     ...
     * @param battler 战斗者
     */
    static onBattlerRemoved(battler: ProjectSceneObject_1): void {
        // -- 如果是角色的话还要清理掉记录关系
     ...
     * @param battler 战斗者
     */
    static callBattlerInitFragmentEvent(battler: ProjectSceneObject_1): void {
        if (!battler.battlerSetting.isExecuteAppearEvent) {
            battler.battlerSetting.isExecuteAppearEvent = true;
            CommandPage.startTriggerFragmentEvent(WorldData.battlerInitEvent, battler, battler);
        }
    }
    //------------------------------------------------------------------------------------------------------
    // 
     ...
     * @param battler 战斗者
     */
    static setBattlerStandby(battler: ProjectSceneObject_1): void {
        // 当前回合行动完毕标识
        battler.battlerSetting.operationComplete = true;
        // 执行片段事件：战斗者开始待机时事件
        CommandPage.startTriggerFragmentEvent(WorldData.startBattlerStandby, battler, battler);
    }
    /**
     * 战斗角色待机效果
     * @param so 
     */
    static clearBattlerStandbyEffect(battler: ProjectSceneObject_1): void {
        // 如果当前回合行动完毕的话
        if (battler.battlerSetting.operationComplete) {
            // 执行片段事件：战斗者结束待机时事件
            CommandPage.startTriggerFragmentEvent(WorldData.stopBattlerStandby, battler, battler);
        }
    }
    /**
     * 让战斗者死亡
     * @param battler 
     */
    static dead(battler: ProjectSceneObject_1): void {
     ...
     */
    static releaseAction(battler: ProjectSceneObject_1, actionID: number, releaseFrame: number, whenCompleteActionID: number, onRelease: Function): void {
    ...
    // 结算奖励
    //------------------------------------------------------------------------------------------------------
    static calcHitReward(winner: ProjectSceneObject_1, onFin: Callback): void {
        ...
        // -- 执行事件
        CommandPage.startTriggerFragmentEvent(WorldData.battleProcess_hitReward, winner, winner, Callback.New(() => {
            // 重置记录的击杀奖励
            this.hitReward = { gold: 0, exp: 0, items: [], equips: [] };
            // 完成时回调
            onFin.run();
     ...
     * @param so 
     */
    static clearHateList(battler: ProjectSceneObject_1, clearAll: boolean = false): void {
     ...
     */
    static removeHateTarget(battler: ProjectSceneObject_1, hateTarget: ProjectSceneObject_1): void {
     ...
     * @param hateTarget 
     */
    static removeHateTargetFromAllList(hateTarget: ProjectSceneObject_1): void {
     ...
     */
    static increaseHate(battler: ProjectSceneObject_1, hateTarget: ProjectSceneObject_1, hateValue: number, ignoreNotInHateList: boolean = false): void {
        // 非敌对势力不允许添加到仇恨列表
     ...
     */
    static increaseHateByHit(fromBattler: ProjectSceneObject_1, targetBattler: ProjectSceneObject_1, hitFrom: Module_技能 | Module_状态, hitValue: number = 0): void {
        // 根据类型决定数值符号
     ...
     * @return [boolean] 
     */
    static addStatus(targetBattler: ProjectSceneObject_1, statusID: number, fromBattler: ProjectSceneObject_1 = null, force: boolean = false): boolean {
        // 获取系统预设的该状态，如果不存在则无法添加
        var systemStatus: Module_状态 = GameData.getModuleData(6, statusID);
        ...
        var thisStatus: Module_状态 = ArrayUtils.matchAttributes(targetBattlerActor.status, { id: statusID }, true)[0];
        if (thisStatus) {
     ...
     * 移除目标状态
     */
    static removeStatus(targetBattler: ProjectSceneObject_1, statusID: number): boolean {
        // 获取系统预设的该状态，如果不存在则无法添加
        var systemStatus: Module_状态 = GameData.getModuleData(6, statusID);
        if (!systemStatus) return false;
        var targetBattlerActor = targetBattler.battlerSetting.battleActor;
     ...
     * 解除全部状态
     */
    static removeAllStatus(battler: ProjectSceneObject_1): void {
        // 动画效果解除
     ...
     * @param fromBattler 来源战斗者
     */
    static removeAllBattlerStatusByFromBattler(fromBattler: ProjectSceneObject_1): void {
     ...
     * @return 是否绑定成功
     */
    private static ifPlayerActorBindingPlayerActorData(battler: ProjectSceneObject_1): boolean {
        // 如果战斗者使用玩家的角色数据
     ...
     * @param so 
     */
    private static onChangeSceneObjectStatus(so: ProjectSceneObject_1): void {
        // 如果是战斗者的话则调用移除
        if (GameBattleHelper.isBattler(so)) {
     ...
     * @param soe 
     */
    private static onChangeSceneObjectCamp(soe: ProjectSceneObject_1) {
        // -- 清理仇恨
        this.clearHateList(soe, true);
        // -- 执行战斗者出现事件
        CommandPage.startTriggerFragmentEvent(WorldData.battlerInitEvent, soe, soe);
    }
    /**
     * 仇恨排序-按照仇恨值从大到小降序
     * @param battler 仇恨列表的拥有者
     */
    private static hateListOrderByDESC(battler: ProjectSceneObject_1) {
