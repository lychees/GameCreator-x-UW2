...
class KeyboardControl {
    ...
    static stop(): void {
        ...
        if (Game.player.sceneObject) Game.player.sceneObject.off(ProjectSceneObject_1.MOVE_OVER, this, this.update);
    }
    //------------------------------------------------------------------------------------------------------
    // 功能
    //------------------------------------------------------------------------------------------------------
        ...
        if (GUI_Setting.IS_KEY(e.keyCode, GUI_Setting.KEY_BOARD.A)) {
            if (GameDialog.isInDialog) {
                ...
                if (Roguelike.current_map == 'world_map') {
                    ...
                    let x = p.x + Roguelike.world_map_ox;
                    let y = p.y + Roguelike.world_map_oy;
                    ...
                    let x = p.x;
                    let y = p.y;
                    let name = ["","market","bar","dry_dock","port","inn","palace","job_house","msc","bank",
     ...
     * @param yGrid 格子坐标点y
     */
    private static moveDirectGrid(xGrid: number, yGrid: number): void {
