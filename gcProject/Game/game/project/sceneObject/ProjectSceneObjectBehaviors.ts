...
class ProjectSceneObjectBehaviors extends SceneObjectBehaviors {
     ...
     */
    so: ProjectSceneObject_1;
    /**
     * 触发事件者
     */
    targetSceneObject: ProjectSceneObject_1;
    /**
     * 执行事件者（派发行为者）
     */
    executor: ProjectSceneObject_1;
    /**
     * 需要恢复的移动行为
     */
    ...
    protected get logicPause(): boolean {
        return (this.executeCommandPageFragment || this.so.isMoving || this.so.isJumping) ? true : false;
    }
    /**
     * 重置：还原到最初始的状态
    ...
    private behavior10(type: number, useVar: number, varID: number, soIndex: number, flip: number = 1) {
        ...
        // 获取目标对象
        var targetSceneObject: ProjectSceneObject_1;
    ...
    private behavior33(enabled: number, w: number, h: number, alpha: number) {
        this.so.shadowEnable = enabled == 1 ? true : false;
        this.so.shadowWidth = w;
        this.so.shadowHeight = h;
        this.so.shadowAlpha = alpha;
    }
    //------------------------------------------------------------------------------------------------------
    // 媒体
    //------------------------------------------------------------------------------------------------------
    ...
    //------------------------------------------------------------------------------------------------------
    /** 忽略不能移动的场合 */
    private behavior37(v: number, keepMoveActWhenCollsionObstacleAndIgnoreCantMove: boolean) {
    ...
    /** 更改动作 */
    private behavior47(useVar: boolean, actionID: number, actionIDVarID: number, once: boolean) {
