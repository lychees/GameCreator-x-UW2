...
class ProjectPlayer extends ClientPlayer {
    ...
    //------------------------------------------------------------------------------------------------------
    /**
     * 事件：卸下了角色的道具 onPlayerActorRemoveItem(inPartyIndex: number, actorDS: DataStructure_inPartyActor, itemIndex: number, item: Module_道具);
     */
    static EVENT_REMOVE_PLAYER_ACTOR_ITEM: string = "ProjectPlayerEVENT_REMOVE_PLAYER_ACTOR_ITEM";
    /**
     * 事件：安装了角色的道具 onPlayerActorCarryItem(inPartyIndex: number, actorDS: DataStructure_inPartyActor, itemIndex: number, removeItem: Module_道具, newItem: Module_道具);
     */
    static EVENT_CARRY_PLAYER_ACTOR_ITEM: string = "ProjectPlayerEVENT_CARRY_PLAYER_ACTOR_ITEM";
    /**
     * 事件：学习了技能 onPlayerActorLearnSkill(inPartyIndex: number, actorDS: DataStructure_inPartyActor, newSkill: Module_技能);
     */
    static EVENT_LEARN_PLAYER_ACTOR_SKILL: string = "ProjectPlayerEVENT_LEARN_PLAYER_ACTOR_SKILL";
    /**
     * 事件：忘记了技能 onPlayerActorForgetSkill(inPartyIndex: number, actorDS: DataStructure_inPartyActor, forgetSkill: Module_技能);
     */
    static EVENT_FORGET_PLAYER_ACTOR_SKILL: string = "ProjectPlayerEVENT_FORGET_PLAYER_ACTOR_SKILL";
    /**
     * 事件：穿戴了装备 onWearPlayerActorEquip(inPartyIndex: number, actorDS: DataStructure_inPartyActor, takeOffEquip: Module_装备, newEquip: Module_装备);
     */
    static EVENT_WEAR_PLAYER_ACTOR_EQUIP: string = "ProjectPlayerEVENT_WEAR_PLAYER_ACTOR_EQUIP";
    /**
     * 事件：卸下了装备 onTakeOffPlayerActorEquip(inPartyIndex: number, actorDS: DataStructure_inPartyActor, takeOffEquip: Module_装备);
     */
     ...
     */
    static getItemIndex(item: Module_道具): number {
     ...
     * @return [number] 位置，不存在返回null
     */
    static getEquipIndex(equip: Module_装备): number {
     ...
     * @param happenEvent [可选] 默认值=true 是否派发相关事件
     */
    static addEquipByInstance(equip: Module_装备, happenEvent: boolean = true): void {
     ...
     * @param happenEvent [可选] 默认值=true 是否派发相关事件
     */
    static addItemByInstance(item: Module_道具, happenEvent: boolean = true): void {
     ...
     * @return [DataStructure_inPartyActor] 
     */
    static getPlayerActorDSByActor(actor: Module_角色): DataStructure_inPartyActor {
     ...
     * @return 所在队伍位置
     */
    static getPlayerActorIndexByActor(actor: Module_角色): number {
    ...
    static initPlayerActor(inPartyIndex: number): void {
        ...
        if (actorDS && actorDS.actor.growUpEnabled) {
            ...
            var classData: Module_职业 = GameData.getModuleData(2, actorDS.actor.class);
    ...
    static addPlayerActorByActorID(actorID: number, lv: number = 1, happenEvent: boolean = true): DataStructure_inPartyActor {
        ...
        var newActor: Module_角色 = GameData.newModuleData(1, actorID, true);
     ...
     * @return [Module_技能] 
     */
    static getPlayerActorSkillBySkillID(inPartyIndex: number, skillID: number): Module_技能 {
     ...
     * @return [boolean] 
     */
    static learnSkillBySkillID(inPartyIndex: number, skillID: number, happenEvent: boolean = true): Module_技能 {
     ...
     * @param inPartyIndex 该角色所在队伍的编号
     * @param happenEvent [可选] 默认值=true 是否派发相关事件
     * @return [Module_技能] 忘记的技能集合
     */
    static forgetAllSkills(inPartyIndex: number, happenEvent: boolean = true): Module_技能[] {
        var actorDS: DataStructure_inPartyActor = ProjectPlayer.getPlayerActorDSByInPartyIndex(inPartyIndex);
        if (!actorDS) return;
     ...
     * @return [Module_装备] 
     */
    static getPlayerActorEquipByPartID(inPartyIndex: number, partID: number): Module_装备 {
     ...
     */
    static wearPlayerActorEquip(inPartyIndex: number, newEquip: Module_装备, happenEvent: boolean = true): { success: boolean, takeOffEquip: Module_装备 } {
        if (newEquip) {
            // -- 查找背包该装备
     ...
     * @param inPartyIndex 该角色所在队伍的索引
     * @param partID 部位ID
     * @return [Module_装备] 卸载出来的装备
     */
    static takeOffPlayerActorEquipByPartID(inPartyIndex: number, partID: number, happenEvent: boolean = true): Module_装备 {
     ...
     * @return [Module_装备] 卸载出来的装备
     */
    static takeOffPlayerActorAllEquips(inPartyIndex: number, happenEvent: boolean = true): Module_装备[] {
        var actorDS = Game.player.data.party[inPartyIndex];
        if (!actorDS) return null;
     ...
     * @return [Module_装备] 
     */
    static getPlayerActorItemByItemIndex(inPartyIndex: number, itemIndex: number): Module_道具 {
        var actorDS = Game.player.data.party[inPartyIndex];
        if (!actorDS) return null;
     ...
     * @param itemIndex 道具位置
     * @param happenEvent [可选] 默认值=true 是否派发相关事件
     * @return { success: boolean, removeItem: Module_道具 }
     */
    static carryPlayerActorItemFromPakcage(inPartyIndex: number, newItem: Module_道具, itemIndex: number, happenEvent: boolean = true): { success: boolean, removeItem: Module_道具 } {
        if (newItem) {
            // -- 查询背包该道具
     ...
     * @return [Module_道具] 
     */
    static unPlayerActorItemByItemIndex(inPartyIndex: number, itemIndex: number, happenEvent: boolean = true): Module_道具 {
        var actorDS = Game.player.data.party[inPartyIndex];
        if (!actorDS) return null;
     ...
     * @param inPartyIndex 角色所在队伍编号
     * @param happenEvent [可选] 默认值=true 是否派发相关事件
     * @return [Module_道具] 卸下来的所有道具集合
     */
    static unPlayerActorAllItems(inPartyIndex: number, itemIndex: number, happenEvent: boolean = true): Module_道具[] {
        var actorDS = Game.player.data.party[inPartyIndex];
        if (!actorDS) return null;
        var arr = [];
     ...
     * 增加经验值
     */
    static increaseExpByIndex(inPartyIndex: number, exp: number): { isLevelUp: boolean, fromLv: number, toLv: number, fromExp: number, toExp: number, learnSkills: Module_技能[] } {
        var actorDS: DataStructure_inPartyActor = ProjectPlayer.getPlayerActorDSByInPartyIndex(inPartyIndex);
        var actor: Module_角色 = actorDS.actor;
        var classData: Module_职业 = GameData.getModuleData(2, actor.class);
        if (!classData) return;
        // 已经最高级则不再允许增加经验值
