...
class ProjectGame extends GameBase {
    ...
    //------------------------------------------------------------------------------------------------------
    /**
     * 事件：卸下了角色的道具 onRemoveActorItem(actor: Module_角色, itemIndex: number, item: Module_道具);
     */
    EVENT_REMOVE_ACTOR_ITEM: string = "GameEVENT_REMOVE_ACTOR_ITEM";
    /**
     * 事件：安装了角色的道具 onCarryActorItem(actor: Module_角色, itemIndex: number, removeItem: Module_道具, newItem: Module_道具);
     */
    EVENT_CARRY_ACTOR_ITEM: string = "GameEVENT_CARRY_ACTOR_ITEM";
    /**
     * 事件：学习了技能 onLearnSkill(actor: Module_角色, newSkill: Module_技能);
     */
    EVENT_LEARN_SKILL: string = "GameEVENT_LEARN_SKILL";
    /**
     * 事件：忘记了技能 onForgetSkill(actor: Module_角色, forgetSkill: Module_技能);
     */
    EVENT_FORGET_SKILL: string = "GameEVENT_FORGET_SKILL";
    /**
     * 事件：穿戴了装备 onWearActorEquip(actor: Module_角色, partID: number, takeOffEquip: Module_装备, newEquip: Module_装备);
     */
    EVENT_WEAR_ACTOR_EQUIP: string = "GameEVENT_WEAR_ACTOR_EQUIP";
    /**
     * 事件：卸下了装备 onTakeOffActorEquip(actor: Module_角色, partID: number, takeOffEquip: Module_装备);
     */
    ...
    */
     ...
     * @return [Module_角色] 
     */
    getActorBySceneObjectIndex(soIndex: number): Module_角色 {
     ...
     * @return [Module_技能] 
     */
    getActorSkillBySkillID(actor: Module_角色, skillID: number): Module_技能 {
     ...
     * @return [Module_技能] 
     */
    actorLearnSkill(actor: Module_角色, skillID: number, happenEvent: boolean = true): Module_技能 {
     ...
     * @param skillID 技能编号
     * @param happenEvent [可选] 默认值=true 是否派发事件
     * @return [Module_技能] 忘却的技能
     */
    actorForgetSkill(actor: Module_角色, skillID: number, happenEvent: boolean = true): Module_技能 {
        var skill = this.getActorSkillBySkillID(actor, skillID);
        if (!skill || !GameData.getModuleData(3, skillID)) return;
     ...
     * @param actor 角色
     * @param happenEvent [可选] 默认值=true 是否派发事件
     * @return [Module_技能] 忘却的技能集合
     */
    actorForgetAllSkills(actor: Module_角色, happenEvent: boolean = true): Module_技能[] {
     ...
     * @return [Module_装备] 
     */
    getActorEquipByPartID(actor: Module_角色, partID: number): Module_装备 {
     ...
     * @return [Module_装备] 
     */
    getActorEquipByEquipID(actor: Module_角色, equipID: number): Module_装备 {
     ...
     */
    wearActorEquip(actor: Module_角色, newEquip: Module_装备, happenEvent: boolean = true): { success: boolean, takeOffEquip: Module_装备 } {
        if (newEquip) {
     ...
     * @return [Module_装备] 
     */
    takeOffActorEquipByPartID(actor: Module_角色, partID: number, happenEvent: boolean = true): Module_装备 {
     ...
     * @return [Module_装备] 
     */
    takeOffActorAllEquips(actor: Module_角色, happenEvent: boolean = true): Module_装备[] {
     ...
     * @param partID 道具位置
     */
    getActorItemByItemIndex(actor: Module_角色, itemIndex: number): Module_道具 {
     ...
     * @return [Module_道具] 
     */
    getActorItemByItemID(actor: Module_角色, itemID: number): Module_道具 {
     ...
     * @param happenEvent [可选] 默认值=true 是否派发事件
     * @return [Module_道具] 卸下的道具
     */
    unActorItemByItemIndex(actor: Module_角色, itemIndex: number, happenEvent: boolean = true): Module_道具 {
     ...
     * @return 更换下来的装备
     */
    carryActorItem(actor: Module_角色, newItem: Module_道具, itemIndex: number, happenEvent: boolean = true): Module_道具 {
     ...
     * @return [number] 
     */
    getLevelUpNeedExp(actor: Module_角色, lv: number): number {
     ...
     * -- 状态加成
     */
    refreshActorAttribute(actor: Module_角色, lv: number): void {
        // 获取数据库预设的数据
     ...
     */
    clacActorAttribute(actor: Module_角色, lv: number, previewChangeEquipMode: boolean = false, previewChangeEquipIndex: number = 0, previewChangeEquip: Module_装备 = null) {
        // 获取数据库预设的数据
        var systemActor = GameData.getModuleData(1, actor.id) as Module_角色;
     ...
     * @return [number] 
     */
    private getGrowValueByLv(actor: Module_角色, growAttrName: string, lv: number): number {
        // -- 获取属性
