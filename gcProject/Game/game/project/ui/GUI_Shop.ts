...
class GUI_Shop extends GUI_12 {
    ...
    public initGoodsList() {
        ...
        for (let i = 0; i < dataArr.length; i++) {
            ...
            let item: Module_道具 | Module_装备 = GameData.getModuleData(moduleID, gooldsID);
            // 道具不存在的情况：忽略
            if (!item) continue;
            data.data = goods;
            data.icon = item.icon;
    ...
    private refreshItemInfo(list: UIList = this.goodsList) {
        ...
        // 已选中道具的情况：显示该道具详情
            ...
            if (this.typeTab.selectedIndex == 0) {
                ...
                var item: Module_道具 | Module_装备 = itemDS.isEquip ? GameData.getModuleData(4, itemDS.equip) : GameData.getModuleData(5, itemDS.item);
            }
            else {
