...
class GUI_BattleReady extends GUI_13 {
     ...
     */
    private actorInSceneBattlers: ProjectSceneObject_1[] = [];
    /**
     * 记录当前更换位置的战斗者
     */
    private currentChangePostionBattler: ProjectSceneObject_1;
    /**
     * 准备出场场景动画集
     */
    private readyInBattleAreaAniArr: Animation[] = [];
    /**
     * 更改朝向阶段
     */
     ...
     * 需要更改朝向的场景对象集
     */
    private needChangeOriSceneObjects: ProjectSceneObject_1[];
    /**
     * 当前更改朝向的场景对象索引
     */
     ...
     * 当前更改朝向的场景对象
     */
    private currentChangeOriSceneObject: ProjectSceneObject_1;
    /**
     * 更改场景对象朝向
     */
        ...
        for (var x = 0; x < gridWidth; x++) {
            ...
            for (var y = 0; y < gridHeight; y++) {
                if (readyInBattleAreaSignArrX[y]) {
                    var effectGrid = GameBattleHelper.createEffectGrid(x, y, 0);
    ...
    private setReadyStep(step: number): void {
        ...
        new SyncTask(task, function (step: number) {
            var fragmentEvent = [WorldData.battleReadyStage1, WorldData.battleReadyStage2, WorldData.battleReadyStage3][step];
            Callback.CallLaterBeforeRender(() => {
                CommandPage.startTriggerFragmentEvent(fragmentEvent, Game.player.sceneObject, Game.player.sceneObject, Callback.New(() => {
    ...
    private putActorInScene(actorIndex: number, grid: Point, actorDS: DataStructure_inPartyActor): void {
        ...
        // 以出厂默认创建创建对象
        var soc = Game.currentScene.addNewSceneObject(1, persetSceneObject) as ProjectSceneObject_1;
     ...
     * @param soc 场景对象
     */
    private backActorFromScene(actorIndex: number, soc: ProjectSceneObject_1): void {
        // 清理已上场记录
        if (this.actorInSceneBattlers[actorIndex]) GUI_BattleReady.readyBattleActorCount--;
     ...
     * @param soc 已上场的战斗者
     */
    private startChangeBattlerPostion(soc: ProjectSceneObject_1) {
        // 设置阶段:更换位置
        this.setReadyStep(1);
        // 记录当前更改位置的对象
        this.currentChangePostionBattler = soc;
        // 调用事件片段：开始更换战斗者位置时事件
        CommandPage.startTriggerFragmentEvent(WorldData.startChangeBattlerPostionEvent, soc, soc);
    }
    /**
     * 停止改变战斗者位置
     * @param soc 已上场的战斗者
     */
    private stopChangeBattlerPostion(soc: ProjectSceneObject_1) {
        // 设置阶段:角色出场
        this.setReadyStep(0);
        // 清空当前更改位置的对象
        this.currentChangePostionBattler = null;
        // 调用事件片段：停止更换战斗者位置时事件
        CommandPage.startTriggerFragmentEvent(WorldData.stopChangeBattlerPostionEvent, soc, soc);
    }
    //------------------------------------------------------------------------------------------------------
    // 放置角色逻辑
    //------------------------------------------------------------------------------------------------------
     ...
     */
    private onMouseUp(selectSceneObject: ProjectSceneObject_1, oldX: number, oldY: number) {
     ...
     */
    private onDragSceneObjectMove(selectSceneObject: ProjectSceneObject_1) {
    ...
    nextSceneObjectCtrlOri(): void {
        ...
        var doNextSceneObjectCtrlOri = () => {
            ...
            // 开始更改战斗者朝向时事件
            CommandPage.startTriggerFragmentEvent(WorldData.startChangeBattlerOriEvent, targetSo, targetSo);
            // 光标设置位置
            GameBattleHelper.cursor.setTo(targetSo.x, targetSo.y);
            // 镜头指向该角色
            GameFunction.cameraMove(1, 0, 0, targetSo.index, true, WorldData.cameraTweenFrame);
            this.currentChangeOriSceneObjectIndex++;
        }
        // 停止更换战斗者朝向时事件
        if (this.currentChangeOriSceneObject && WorldData.stopChangeBattlerOriEvent) {
            CommandPage.startTriggerFragmentEvent(WorldData.stopChangeBattlerOriEvent, this.currentChangeOriSceneObject, this.currentChangeOriSceneObject, Callback.New(() => {
