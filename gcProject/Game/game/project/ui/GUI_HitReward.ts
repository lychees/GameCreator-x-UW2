...
class GUI_HitReward extends GUI_22 {
    static rewardBattler: ProjectSceneObject_1;
    static rewardActor: Module_角色;
    static increaseExpRes: { isLevelUp: boolean, fromLv: number, toLv: number, fromExp: number, toExp: number, learnSkills: Module_技能[] };
    ...
    private taskShowGetExp(): void {
        ...
        for (var lv = increaseExpRes.fromLv; lv <= increaseExpRes.toLv; lv++) {
            ...
            new SyncTask(this.getExpAnimationTask, (lv: number, thisLvStartExp: number, thisLvEndExp: number, thisLvNeedExp: number) => {
                ...
                if (lv != increaseExpRes.fromLv) {
                    // 播放升级动画
                    GUI_HitReward.rewardBattler.playAnimation(21, false, true);
                }
    ...
    private taskShowGetItems(): void {
        ...
        for (var i = 0; i < dropItems.length; i++) {
            ...
            var dropItem: Module_道具 = GameData.getModuleData(5, dropItemInfo.itemID);
