...
class GUI_Party extends GUI_11 {
    ...
    private refreshActorDataPanel() {
        ...
        var classData: Module_职业 = GameData.getModuleData(2, selectedActor.class);
    ...
    private refreshActorSkillPanel() {
        ...
        for (var i = 0; i < selectedActorDS.actor.skills.length; i++) {
            ...
            // 获取背包的道具DS格式
            var skill: Module_技能 = selectedActorDS.actor.skills[i];
    ...
    private refreshActorEquips() {
        ...
        // 遍历部件
        for (var i = 0; i < 5; i++) {
            var equip: Module_装备 = Game.getActorEquipByPartID(selectedActorDS.actor, i);
            // 创建对应的背包物品项数据，该项数据由系统自动生成
    ...
    private refreshEquipPackageList() {
        ...
        var classData: Module_职业 = GameData.getModuleData(2, classID);
        if (!classData) return;
    ...
    private refreshActorItems() {
        ...
        for (var i = 0; i < actorItemMax; i++) {
            var item: Module_道具 = selectedActorDS.actor.items[i];
            // 创建对应的背包物品项数据，该项数据由系统自动生成
    ...
    private refreshDescribe(): void {
        ...
        if (UIList.focus == this.actorSkillList) {
            ...
            var skill = itemData.data as Module_技能;
            if (skill) {
                name = skill.name;
        ...
        // 焦点在装备栏的情况下
            ...
            var equip: Module_装备 = this.actorEquipList.selectedItem.data;
            if (equip) {
                name = equip.name;
        ...
        // 焦点在物品栏的情况下
            ...
            var item: Module_道具 = this.actorItemList.selectedItem.data;
            if (item) {
                name = item.name;
