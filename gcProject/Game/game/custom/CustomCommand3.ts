...
module CommandExecute {
    ...
    export function customCommand_5001(commandPage: CommandPage, cmd: Command, trigger: CommandTrigger, triggerPlayer: ClientPlayer, playerInput: any[], cp: CustomCommandParams_5001): void {
        ...
        // 执行进入战斗前事件处理
        CommandPage.startTriggerFragmentEvent(WorldData.beforeStartBattleEvent, trigger.trigger, trigger.executor, Callback.New(() => {
            // 主角变为光标
    ...
    export function customCommand_5002(commandPage: CommandPage, cmd: Command, trigger: CommandTrigger, triggerPlayer: ClientPlayer, playerInput: any[], cp: CustomCommandParams_5002): void {
        ...
        GameBattle.stop(() => {
            // 恢复战前的状态
            var playerSo: ProjectSceneObject_1 = Game.player.sceneObject;
            ...
            CommandPage.startTriggerFragmentEvent(WorldData.afterEndBattleEvent, Game.player.sceneObject, Game.player.sceneObject, Callback.New(() => {
    ...
    export function customCommand_5008(commandPage: CommandPage, cmd: Command, trigger: CommandTrigger, triggerPlayer: ClientPlayer, playerInput: any[], cp: CustomCommandParams_5008): void {
        ...
        var actorData: Module_角色 = GameData.getModuleData(1, actorID);
        if (!actorData) return;
        if (!GameBattleHelper.isInBattle) return;
        // 如果与触发者阵营相同则判断触发者是否是战斗者
        if (cp.camp == 2 && !GameBattleHelper.isBattler(trigger.trigger as ProjectSceneObject_1)) return;
        // 如果与执行者阵营相同则判断执行者是否是战斗者
        if (cp.camp == 3 && !GameBattleHelper.isBattler(trigger.executor as ProjectSceneObject_1)) return;
        if (cp.gridUseVar) {
        ...
        switch (cp.camp) {
            ...
            case 1:
                ...
                break;
            case 2:
                soc.battlerSetting.battleCamp = (trigger.trigger as ProjectSceneObject_1).battlerSetting.battleCamp;
                break;
            case 3:
                soc.battlerSetting.battleCamp = (trigger.executor as ProjectSceneObject_1).battlerSetting.battleCamp;
                break;
        }
    ...
    export function customCommand_6001(commandPage: CommandPage, cmd: Command, trigger: CommandTrigger, triggerPlayer: ClientPlayer, playerInput: any[], p: CustomCommandParams_6001): void {
        ...
        if (p.attributeRand && p.equipRandSetting.length > 0 && p.symbol == 0) {
            ...
            // 生成装备
            for (var n = 0; n < num; n++) {
                var newEquip: Module_装备 = GameData.newModuleData(4, equipID, true);
                for (var i = 0; i < p.equipRandSetting.length; i++) {
